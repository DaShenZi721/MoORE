...
class CasualOutputLayer(LLMOutput):
    ...
    def forward(self, data: torch.Tensor, task_ids: torch.Tensor = None) -> torch.Tensor:
...
class ClassificationOutputLayer(LLMOutput):
    ...
    def forward(self, data: torch.Tensor, task_ids: torch.Tensor = None) -> torch.Tensor:
        scores = self.score_(data.to(torch.float32))
        
        # 因为mnli的label是0,1,2，所以需要特殊处理一下
        mask = (task_ids != TASK_IDS['glue:mnli']).unsqueeze(-1)
        
        # 使用 masked_fill_ 将符合条件的指数置为 -inf
        scores[:, :, 2].masked_fill_(mask, -torch.inf)
...
class OutputLayer(torch.nn.Module):
    ...
    def forward(
        self, data: torch.Tensor, input_args: LLMModelInput, task_ids: torch.Tensor = None,
    ) -> List[LLMModelOutput]:
        outputs = []
            ...
            outputs.append(
                LLMModelOutput(
                    ...
                    logits=layer.forward(
                        ...
                        task_ids[start_idx:end_idx] if task_ids is not None else None,
                    ),
                    loss_fn_=layer.loss,
                )
            )
...
def init_lora_layer_weight(
    ...
    elif isinstance(lora_config, MoOREConfig):
        model_prefix_name = "moore"
        moe_layer_name_list = list(all_state_dict.keys())
        moe_init_strategy = "svd"
    ...
    assert len(moe_layer_name_list) == 0 or moe_init_strategy in ["plugin", "fused_mlp", "svd"]
    ...
    for proj_name, lora_linear in all_state_dict.items():
        ...
        if moe_init_strategy == "svd":
            if lora_weights is None:
                ...
                gate_params = [None] * 5
                gate_params[0] = lora_weights.get(f"{module_name}.moe_gate.weight", None)
                gate_params[2] = lora_weights.get(f"{module_name}.moe_gate.task_embedding", None)
                gate_params[3] = lora_weights.get(f"{module_name}.moe_gate.task_linear", None)
                gate_params[4] = lora_weights.get(f"{module_name}.moe_gate.up.weight", None)
                hra_u_in = [lora_weights.get(f"{module_name}.hra_u_in.{i}", None) for i in range(lora_config.lora_r_)]
                
            lora_linear.init_svd_weight(lora_config, (gate_params, hra_u_in, hra_u_out))
        else:
...
def get_lora_layer_weight(
    ...
    if isinstance(lora_config, MixLoraConfig):
        ...
        model_prefix_name = "moore"
        gate_layer_name = (
            f"moore.layers.{transformer_layer.layer_id_}.mlp.moe_gate.weight"
        )
        moe_layer_name_list = list(all_state_dict.keys())
        adapter_name = 'svd'
    elif isinstance(lora_config, LoraMoeConfig):
    ...
    for proj_name, lora_linear in all_state_dict.items():
        ...
        module_name = f"{model_prefix_name}.layers.{transformer_layer.layer_id_}.{module_name}.{proj_name}"
        if adapter_name == 'svd':
            svd_obj = lora_linear.svds_.get(lora_config.adapter_name, None)
            if svd_obj is not None:
                lora_weights[f"{module_name}.moe_gate.weight"] = svd_obj.gate.weight
                lora_weights[f"{module_name}.moe_gate.task_embedding"] = svd_obj.task_embedding.weight
                lora_weights[f"{module_name}.moe_gate.task_linear"] = svd_obj.task_linear.weight
                lora_weights[f"{module_name}.moe_gate.up.weight"] = svd_obj.gate_up.weight
                for i in range(lora_config.lora_r_):
                    lora_weights[f"{module_name}.hra_u_in.{i}"] = svd_obj.hra_u_in[i]
        else:
            if proj_name in moe_layer_name_list:
                moe_layer = (
...
class LLMModel(torch.nn.Module):
    ...
    def _prepare_inputs(
        ...
        # prepare task ids
        if input_args.batch_task_ids_ is not None:
            task_ids = torch.tensor(
                input_args.batch_task_ids_, dtype=torch.int64, device=self.device_
            )
        else:
            task_ids = None

        return input_ids, inputs_embeds, attention_mask, causal_mask, cache_position, task_ids
    ...
    def _call_decoder_stack(
        ...
        output_router_logits_: Optional[bool] = False,
    ):
        # decoder layers
    ...
    def forward(
        ...
        input_ids, inputs_embeds, attention_mask, causal_mask, cache_position, task_ids = (
        ...
        input_args.batch_task_ids_ = None
        ...
        for idx, lora_config in enumerate(input_args.batch_configs_):
            output_router_logits_ = getattr(self.adapter_configs_[lora_config.adapter_name_], 'output_router_logits_', False)
        ...
        # calculate loss
        output = self.output_(hidden_states, input_args, task_ids=task_ids)
        ...
        for idx, lora_config in enumerate(input_args.batch_configs_):
            ...
            if getattr(self.adapter_configs_[lora_config.adapter_name_], 'output_router_logits_', False) and len(all_router_logits[idx]) > 0:
    ...
    def from_pretrained(
        ...
        logging.info(f'{name_or_path} total parameters: {llm_model.num_parameters()}')
    ...
    def init_adapter(
        ...
        # Patch for MoORE
        if isinstance(config, MoOREConfig) and config.act_fn_ is None:
            config.act_fn_ = self.config_.hidden_act_

        self.adapter_configs_[config.adapter_name] = config
        # init output layer
        if config.task_name is not None and ';' in config.task_name:
            task_names = config.task_name.split(';')
        else:
            task_names = [config.task_name]
        # 因为mnli的label是0,1,2，所以需要特殊处理一下
        if 'glue:mnli' in task_names:
            task_names = ['glue:mnli']
        if task_names[0] in task_dict and isinstance(
            task_dict[task_names[0]], SequenceClassificationTask
        ):
            output_layer = ClassificationOutputLayer(
                **task_dict[task_names[0]].init_kwargs(),
        ...
        if type(config) is not AdapterConfig:
            ...
            for layer_idx, transformer_layer in enumerate(self.model_.layers_):
                init_lora_layer_weight(transformer_layer, self.config_, config, weight, layer_idx)
        else:
    ...
    def get_adapter_weight_dict(self, adapter_name: str) -> Dict[str, torch.Tensor]:
        ...
        for layer_idx, transformer_layer in enumerate(self.model_.layers_):
            get_lora_layer_weight(transformer_layer, lora_config, lora_weight_dict, layer_idx=layer_idx)
