...
from .adapters import MixLoraConfig, MoOREConfig
...
def _prepare_tasks(model, tokenizer, configs):
    for config in configs:
        ...
        if isinstance(model.adapter_configs_[config.adapter_name], MixLoraConfig):
        ...
        if isinstance(model.adapter_configs_[config.adapter_name], MoOREConfig):
            for layer in model.model_.layers_:
                if config.adapter_name in layer.mlp_.mlp_.w1_.svds_:
                    layer.mlp_.mlp_.w1_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.mlp_.mlp_.w1_.svds_[config.adapter_name].profiler = {}
                if config.adapter_name in layer.mlp_.mlp_.w2_.svds_:
                    layer.mlp_.mlp_.w2_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.mlp_.mlp_.w2_.svds_[config.adapter_name].profiler = {}
                if config.adapter_name in layer.mlp_.mlp_.w3_.svds_:
                    layer.mlp_.mlp_.w3_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.mlp_.mlp_.w3_.svds_[config.adapter_name].profiler = {}
                if config.adapter_name in layer.self_attn_.wq_.svds_:
                    layer.self_attn_.wq_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.self_attn_.wq_.svds_[config.adapter_name].profiler = {}
                if config.adapter_name in layer.self_attn_.wk_.svds_:
                    layer.self_attn_.wk_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.self_attn_.wk_.svds_[config.adapter_name].profiler = {}
                if config.adapter_name in layer.self_attn_.wv_.svds_:
                    layer.self_attn_.wv_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.self_attn_.wv_.svds_[config.adapter_name].profiler = {}
                if config.adapter_name in layer.self_attn_.wo_.svds_:
                    layer.self_attn_.wo_.svds_[config.adapter_name].router_profile_ = config.router_profile
                    layer.self_attn_.wo_.svds_[config.adapter_name].profiler = {}
...
def _dispatch_task_in(tokenizer, configs, concurrent_jobs, max_seq_len):
    ...
    for config in configs:
        ...
        for idx in range(config.batch_start_idx_, config.batch_end_idx_):
            ...
            task_id = config.data_[idx].task_id
    ...
    return (
        ...
        LLMModelInput(
            ...
            batch_task_ids_=batch_task_ids,
